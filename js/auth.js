/**
 * Feishu Auth v9.1 - Strict gate, Feishu-only access
 * Browser access blocked, only Feishu embedded browser allowed
 * Flow: Feishu env check → SDK auth → AIPA verify → show content
 * Adds "Open in Feishu" button for browser users
 */
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (!isInFeishu) {
    ...
    if (overlay) {
      ...
      btn.onclick = function() {
        // Open via Feishu H5 app applink — guaranteed to open in Feishu
        var applink = 'https://applink.feishu.cn/client/app/open?app_id=' + FeishuAuthHelper.APP_ID;
  ...
  // Step 3: In Feishu, do SDK auth
  console.log('[FeishuAuth] In Feishu, starting SDK auth');
  if (titleEl) titleEl.textContent = '正在验证身份...';
  ...
  var timeoutId = setTimeout(function() {
    timedOut = true;
    if (titleEl) titleEl.textContent = '验证超时';
    if (descEl) descEl.textContent = '请下拉刷新页面重试';
  }, 20000);
  ...
  function doAuth() {
    ...
    window.tt.requestAuthCode({
      ...
      success: function(res) {
        ...
        // Exchange code for user via AIPA
