/**
 * Feishu Auth Helper v16 - Universal auth with smart cache
 * Flow: sessionStorage → URL code → SDK requestAuthCode → OAuth redirect → fallback
 * Fix: ignore cached fallback users (u_xxx), only trust real Feishu auth
 */
const FeishuAuthHelper = {
  ...
  async getUser() {
    ...
    // Step 1: Check sessionStorage cache - but ONLY real Feishu users
    try {
      const cached = sessionStorage.getItem('feishu_user');
      if (cached) {
        const parsed = JSON.parse(cached);
        // Only trust cache if it's a real Feishu user (open_id starts with "ou_")
        // Ignore fallback users (starts with "u_") or empty username
        if (parsed.userId && parsed.userId.startsWith('ou_')) {
          this._user = parsed;
          console.log('[Auth] Cache hit (real user):', this._user.userId);
          return this._user;
        }
        console.log('[Auth] Cache has fallback user, ignoring');
