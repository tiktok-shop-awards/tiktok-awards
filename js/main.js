...
const CHUNK_VERSION = 'member-add-official-v1';
...
function renderPodium(top3, containerId, title) {
  ...
  function renderTiedCard(group) {
    ...
    if (total <= SHOW_ALL_THRESHOLD) {
      ...
      noteHtml = `<div class="podium-note">Too many outstanding winners to name individually — congratulations to everyone!</div>`;
    }
    return `
      <div class="podium-item ${getClass(group.rank)} podium-tied">
        <div class="podium-medal">${getMedal(group.rank)}</div>
        <div class="podium-rank">Tied for ${group.rank}</div><div class="podium-rank-sub">(${total} people)</div>
...
function showShareModal(projectName, teamAward, bonus, reason, members, awardType) {
  ...
  // Format members list - adaptive layout (v2)
  let memberList = '';
  let membersModeClass = 'mode-single';
  const memberCount = Array.isArray(members) ? members.length : 0;
  
  if (awardType === 'individual') {
    // 个人奖：单人模式，显示名字+部门+角色
    membersModeClass = 'mode-single';
    const member = Array.isArray(members) ? members[0] : { name: members };
    const name = member.name || member;
    const dept = member.department || member.dept || '';
    const role = member.role || '';
    memberList = `
      <div class="member-name">${name}</div>
      ${dept ? `<div class="member-dept">${dept}</div>` : ''}
      ${role ? `<div class="member-role">${role.toUpperCase()}</div>` : ''}
    `;
    ...
    if (memberCount === 1) {
      ...
      const m = members[0];
      const name = m.name || m;
      const dept = m.department || m.dept || '';
      memberList = `
        <div class="member-name">${name}</div>
        ${dept ? `<div class="member-dept">${dept}</div>` : ''}
      `;
    } else if (memberCount <= 5) {
      membersModeClass = 'mode-few';
      memberList = members.map(m => {
        const name = m.name || m;
        const dept = m.department || m.dept || '';
        return `<div class="member-item"><div class="member-name">${name}</div>${dept ? `<div class="member-dept">${dept}</div>` : ''}</div>`;
      }).join('');
    } else if (memberCount <= 20) {
      membersModeClass = 'mode-medium';
      memberList = members.map(m => {
        const name = m.name || m;
        const dept = m.department || m.dept || '';
        return `<div class="member-item"><div class="member-name">${name}</div>${dept ? `<div class="member-dept">${dept}</div>` : ''}</div>`;
      ...
      membersModeClass = 'mode-many';
      memberList = members.map(m => {
        const name = m.name || m;
        return `<div class="member-item"><div class="member-name">${name}</div></div>`;
      }).join('');
    } else {
      membersModeClass = 'mode-wall';
      memberList = members.map(m => {
        const name = m.name || m;
        return `<div class="member-item"><div class="member-name">${name}</div></div>`;
      }).join('');
  ...
  const membersCountLabel = memberCount > 0 ? ` <span class="count">· ${memberCount} ${memberCount === 1 ? 'person' : 'people'}</span>` : '';
  ...
  posterPreview.innerHTML = `
    <div id="poster-wrapper" style="overflow:hidden;position:relative;">
      <div class="poster-v2-container" id="poster-content">
        <div class="poster-v2-corner-br"></div>
        <div class="poster-v2-top">
          <div class="poster-v2-brand">Global E-Commerce · ${awardTitle}</div>
          <div class="poster-v2-title-row">
            <div class="poster-v2-title">${projectName}</div>
            ${bonusFormatted ? `
            <div class="poster-v2-bonus">
              <div class="poster-v2-bonus-num">${bonusFormatted}</div>
              <div class="poster-v2-bonus-label">Award Bonus</div>
            </div>` : ''}
          </div>
          <div class="poster-v2-divider"></div>
        </div>
        <div class="poster-v2-body">
          ...
          <div class="poster-v2-members-section">
            ...
            <div class="poster-v2-members ${membersModeClass}">
              ${memberList}
            </div>
          </div>
        </div>
        <div class="poster-v2-footer">
          <img class="poster-v2-logo" src="${tiktokLogoSrc}" alt="TikTok" />
          <div class="poster-v2-footer-brand">TikTok Shop</div>
        </div>
      </div>
    </div>
  `;
