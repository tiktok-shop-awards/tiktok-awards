...
async function loadData(level, region = null, year = null) {
  try {
    ...
    if (level === 'global') {
      dataFile = 'data/global.json?v=20260710f';
    } else if (level === 'regional' && region) {
      dataFile = 'data/' + region + '.json?v=20260710f';
    } else if (level === 'fs') {
      dataFile = 'data/fs.json?v=20260710f';
    } else if (level === 'pop') {
      dataFile = 'data/pop.json?v=20260710f';
    } else if (level === 'departmental') {
      dataFile = 'data/departmental.json?v=20260710f';
    }
...
async function loadRankings(year = null) {
  try {
    const response = await fetch('data/rankings.json?v=20260710f');
...
async function loadSearchData() {
  try {
    const [global, us, eu, sea, latam, rankings, departmental, fs, pop, nameMapData] = await Promise.all([
      fetch('data/global.json?v=20260710f').then(r => r.json()).catch(() => null),
      fetch('data/us.json?v=20260710f').then(r => r.json()).catch(() => null),
      fetch('data/eu.json?v=20260710f').then(r => r.json()).catch(() => null),
      fetch('data/sea.json?v=20260710f').then(r => r.json()).catch(() => null),
      fetch('data/latam.json?v=20260710f').then(r => r.json()).catch(() => null),
      fetch('data/rankings.json?v=20260710f').then(r => r.json()).catch(() => null),
      fetch('data/departmental.json?v=20260710f').then(r => r.json()).catch(() => null),
      fetch('data/fs.json?v=20260710f').then(r => r.json()).catch(() => null),
      fetch('data/pop.json?v=20260710f').then(r => r.json()).catch(() => null),
      fetch('data/name-map.json?v=20260710f').then(r => r.json()).catch(() => null)
    ]);
    nameMap = nameMapData || {};
