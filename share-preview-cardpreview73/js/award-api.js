...
const AwardAPI = {
  ...
  /**
   * List Feishu group chats visible to the signed-in user.
   * Expected response: { success: true, data: { chats: [{ chat_id, name, avatar }] } }
   */
  async listFeishuShareChats(query = '') {
    const url = new URL(`${this.BASE_URL}/api/feishu/share/chats`);
    ...
    return data?.chats || data?.items || [];
  },

  /**
   * Send one award as a Feishu Card 2.0 message.
   * The backend owns user tokens and constructs the final card JSON.
   */
  async sendAwardCardToChat(chatId, award) {
    const user = await this._waitForCachedFeishuUser(5000);
    const res = await fetch(`${this.BASE_URL}/api/feishu/share/award-card`, {
      method: 'POST',
      credentials: 'include',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        chat_id: chatId,
        sender_open_id: user?.userId || '',
        dry_run: true,
        award
      }),
